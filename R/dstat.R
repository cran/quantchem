...
result = c(mean(x,na.rm=na.rm),ttest$conf.int,ttest$statistic,ttest$p.value);
names(result) = c("Mean","Lower","Upper","t","Pr(>t)");
return(result);
}

medians = function(x) {
wtest = wilcox.test(x,na.rm=na.rm,conf.int=TRUE,mu=expected,conf.level=conf.level);
result = c(median(x,na.rm=na.rm),wtest$conf.int,wtest$statistic,wtest$p.value);
...
vars = function(x) {
dtest = dixon.test(x)
result = c(var(x,na.rm=na.rm),sd(x,na.rm=na.rm),sd(x,na.rm=na.rm)/sqrt(length(x)),
dtest$statistic,dtest$p.value);
...
n = length(x)
gtest = grubbs.test(x)
rsdval = c(var(x),((n-1)/qchisq((1+conf.level)/2,n-1)*var(x)),((n-1)/qchisq((1-conf.level)/2,n-1)*var(x)));
rsdval = sqrt(rsdval);
...
result = c(range(x),diff(range(x)),IQR(x),mad(x),stest$statistic,stest$p.value);
names(result) = c("Min","Max","Range","IQR","MAD","W","Pr(<W)");
return(result);
}
...
res$vartest = rbind(res$vartest,c(1/vtest$estimate,1/vtest$conf.int,1/vtest$statistic,vtest$p.value))
else
...
rownames(res$wtest)[nrow(res$wtest)]=paste(names(x)[i],names(x)[j],sep="-");
}
}
colnames(res$vartest)=c("Ratio","Lower","Upper","F","Pr(>F)");
colnames(res$ttest)=c("Diff","Lower","Upper","t","Pr(>t)");
...
if (!is.null(res$vartest)) res$vartest = sorted(res$vartest,5);
if (!is.null(res$ttest)) res$ttest = sorted(res$ttest,5);
if (!is.null(res$atest)) res$atest = sorted(res$atest,5);
if (!is.null(res$wtest)) res$wtest = sorted(res$wtest,5);
}
...
if (!is.null(res$vartest)) { cat("\nRatios of variances:\n"); printCoefmat(res$vartest); }
if (!is.null(res$ttest)) { cat("\nDifferences between means:\n"); printCoefmat(res$ttest); }
if (!is.null(res$atest)) { cat("\nDifferences in scale:\n"); printCoefmat(res$atest); }
if (!is.null(res$wtest)) { cat("\nDifferences in locations:\n"); printCoefmat(res$wtest); }
if (!is.null(res$anova)) 
cat(paste("\nANOVA: F =",format(res$anova[1,4]),"p-value =",format.pval(res$anova[1,5]),"\n"));
if (!is.null(res$kruskal)) 
cat(paste("Kruskal-Wallis test: chi-squared =",format(res$kruskal$statistic),"p-value =",format.pval(res$kruskal$p.value),"\n"));
if (!is.null(res$bartlett)) 
cat(paste("\nBartlett test : K-Squared =",format(res$bartlett$statistic),"p-value =",format.pval(res$bartlett$p.value),"\n"));
if (!is.null(res$fligner)) 
cat(paste("Fligner-Killeen test: chi-squared =",format(res$fligner$statistic),"p-value =",format.pval(res$fligner$p.value),"\n"));
invisible(res);
